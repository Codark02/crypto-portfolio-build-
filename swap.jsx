...
export default function CryptoSwap() {
  ...
  // Hidden Relay.link swap execution
  const executeRelaySwap = async (tokenIn, tokenOut, amountIn) => {
    try {
      console.log('Executing hidden Relay.link swap...');
      
      const chainId = parseInt(NETWORKS[selectedNetwork].chainId, 16);
      
      // Get quote from Relay.link (hidden)
      const quoteResponse = await fetch(`${RELAY_CONFIG.baseUrl}/v1/quotes`, {
        method: 'POST',
        headers: {
      ...
      // Get transaction data from Relay.link (hidden)
      const transactionResponse = await fetch(`${RELAY_CONFIG.baseUrl}/v1/transactions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${RELAY_CONFIG.apiKey}`
      ...
      console.log('Relay transaction data received');
      ...
      console.error('Relay swap failed:', error);
      throw error;
    }
  };
  ...
  const executeBuy = async () => {
    ...
    if (!selectedBuyCoin) {
      setError('Please select a cryptocurrency to buy');
      return;
    }
    ...
    try {
      setLoading(true);
      setTxStatus('Processing buy transaction...');
      
      // Get token addresses
      const tokenIn = USDC_ADDRESS; // Buy with USDC
      const tokenOut = getTokenAddress(selectedBuyCoin.symbol) || WETH_ADDRESS;
      
      // Convert amount to wei (USDC has 6 decimals)
      const amountIn = ethers.parseUnits(amount.toString(), 6);
      
      console.log('Executing buy:', { tokenIn, tokenOut, amountIn: amountIn.toString() });
      
      // Execute hidden Relay.link swap
      const txHash = await executeRelaySwap(tokenIn, tokenOut, amountIn);
      ...
      setTxStatus(`Successfully bought ${selectedBuyCoin.name}!`);
      alert(`Successfully bought ${selectedBuyCoin.name}!\nTransaction: ${txHash}`);
  ...
  const executeSell = async () => {
    ...
    if (!selectedSellCoin) {
      setError('Please select a cryptocurrency to sell');
      return;
    }
    ...
    // Check balance
    const currentBalance = balances[selectedSellCoin.symbol] || 0;
    if (amount > currentBalance) {
      setError(`Insufficient balance. You have ${currentBalance.toFixed(6)} ${selectedSellCoin.symbol}`);
      return;
    }
    
    try {
      setLoading(true);
      setTxStatus('Processing sell transaction...');
      
      // Get token addresses
      const tokenIn = getTokenAddress(selectedSellCoin.symbol) || WETH_ADDRESS;
      const tokenOut = USDC_ADDRESS; // Get USDC
      
      // Convert amount to wei
      const decimals = selectedSellCoin.symbol === 'USDC' || selectedSellCoin.symbol === 'USDT' ? 6 : 18;
      const amountIn = ethers.parseUnits(amount.toString(), decimals);
      
      console.log('Executing sell:', { tokenIn, tokenOut, amountIn: amountIn.toString() });
      ...
      setTxStatus(`Successfully sold ${selectedSellCoin.name}!`);
      alert(`Successfully sold ${selectedSellCoin.name}!\nTransaction: ${txHash}`);
      ...
      setError(`Sell failed: ${error.message}`);
    } finally {
      setLoading(false);
      setTxStatus('');
    }
